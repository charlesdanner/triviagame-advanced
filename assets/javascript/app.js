$(document).ready(function () {
    ...
    function nextQuestion() {
        if (currentQuestion === 6) {
            setTimeout(function () {                //this function is for verifying if the game is over or not.
                clearInterval(domIntervalDisplay)   //clears interval timers just in case they're still running, hides the verdict tab on the dom,
                clearTimeout(questionTimerOut)      //shows the reset game button, shows the results and populates the results with however many questions were right or wrong.
                $(".verdict").hide();
                $("#reset-game").show();
                $(".results").show();
    ...
    $("#reset").on("click", function (e) {
        ...
        incorrect = 0;
        unanswered = 0;
        timeLeft = 20;              //when the game is over the reset button resets the variables that have been incremented throughout the game, hides itself and runs the function to populate the DOM with question0 to start the game over again.
        currentQuestion = 0;
        $("#reset-game").hide();
        $(".results").hide();
        newQuestion();
